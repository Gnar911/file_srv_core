#include "mmap_decoder_row_index.h"
...
DecoderRowIndexMmap::DecoderRowIndexMmap(std::string base_path,
                                         uint32_t segment_capacity)
    : base_path_(std::move(base_path)),
      segment_capacity_(segment_capacity) {}

std::vector<std::string> DecoderRowIndexMmap::planned_segment_paths_() const {
    std::vector<std::string> out;
...
int32_t DecoderRowIndexMmap::open_segment_(uint32_t seg_idx) {
    Segment seg;
...
int32_t DecoderRowIndexMmap::ensure_segment_(uint64_t global_offset) {
...
int32_t DecoderRowIndexMmap::open_and_init() {
    close_and_finalize();
    if (segment_capacity_ == 0) {
        return file_service::mmap::error_code::kHeaderZeroMetadata;
    }
...
int32_t DecoderRowIndexMmap::write_at(uint64_t global_offset, uint32_t row_index) {
...
void DecoderRowIndexMmap::publish_progress(uint64_t written_total, bool done) {
    (void)done;
...
void DecoderRowIndexMmap::close_and_finalize() {
...
std::vector<std::string> DecoderRowIndexMmap::segment_paths() const {
    if (!segments_.empty()) {
...
int32_t DecoderRowIndexMmap::read_total_count(uint64_t& out_total_count) const {
    out_total_count = 0;
...
int32_t DecoderRowIndexMmap::read_value(uint64_t global_offset, uint32_t& out_row_index) const {
    const auto paths = segment_paths();
    return read_u32(paths, global_offset, out_row_index);
}

std::vector<uint32_t> DecoderRowIndexMmap::read_page(int64_t first, int64_t last) const {
