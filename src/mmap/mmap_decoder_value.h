...
class DecoderValueMmap {
public:
    explicit DecoderValueMmap(std::string base_path,
                              uint32_t segment_capacity = 1'000'000U);

    int32_t open_and_init();
    int32_t write_at(uint64_t global_offset, double value);
    ...
    int32_t read_value(uint64_t global_offset, double& out_value) const;
    std::vector<double> read_page(int64_t first, int64_t last) const;
