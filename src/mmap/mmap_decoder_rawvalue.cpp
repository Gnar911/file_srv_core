#include "mmap_decoder_rawvalue.h"
...
int32_t read_i64(const std::vector<std::string>& paths,
    ...
    const size_t off = sizeof(SoAHeader) + static_cast<size_t>(local_idx) * sizeof(int64_t);
    if (off + sizeof(int64_t) > h.size) {
        mmap_close(h);
        return file_service::mmap::error_code::kReadOutOfRange;
    }

    std::memcpy(&out_value, reinterpret_cast<const uint8_t*>(h.addr) + off, sizeof(int64_t));
    mmap_close(h);
    return 0;
}
...
DecoderRawValueMmap::DecoderRawValueMmap(std::string base_path,
                                         uint32_t segment_capacity)
    : base_path_(std::move(base_path)),
      segment_capacity_(segment_capacity) {}

std::vector<std::string> DecoderRawValueMmap::planned_segment_paths_() const {
    std::vector<std::string> out;
...
int32_t DecoderRawValueMmap::open_segment_(uint32_t seg_idx) {
    Segment seg;
...
int32_t DecoderRawValueMmap::ensure_segment_(uint64_t global_offset) {
...
int32_t DecoderRawValueMmap::open_and_init() {
    close_and_finalize();
    if (segment_capacity_ == 0) {
        return file_service::mmap::error_code::kHeaderZeroMetadata;
    }
...
int32_t DecoderRawValueMmap::write_at(uint64_t global_offset, int64_t value) {
...
void DecoderRawValueMmap::publish_progress(uint64_t written_total, bool done) {
    (void)done;
...
void DecoderRawValueMmap::close_and_finalize() {
...
std::vector<std::string> DecoderRawValueMmap::segment_paths() const {
    if (!segments_.empty()) {
...
int32_t DecoderRawValueMmap::read_total_count(uint64_t& out_total_count) const {
    out_total_count = 0;
...
int32_t DecoderRawValueMmap::read_value(uint64_t global_offset, int64_t& out_value) const {
    const auto paths = segment_paths();
    return read_i64(paths, global_offset, out_value);
}

std::vector<int64_t> DecoderRawValueMmap::read_page(int64_t first, int64_t last) const {
