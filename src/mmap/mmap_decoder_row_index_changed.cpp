#include "mmap_decoder_row_index_changed.h"
...
int32_t read_u32(const std::vector<std::string>& paths,
    ...
    const size_t off = sizeof(SoAHeader) + static_cast<size_t>(local_idx) * sizeof(uint32_t);
    if (off + sizeof(uint32_t) > h.size) {
        mmap_close(h);
        return file_service::mmap::error_code::kReadOutOfRange;
    }

    std::memcpy(&out_value, reinterpret_cast<const uint8_t*>(h.addr) + off, sizeof(uint32_t));
    mmap_close(h);
    return 0;
}
...
DecoderRowIndexChangedMmap::DecoderRowIndexChangedMmap(std::string base_path,
                                                       uint32_t segment_capacity)
    : base_path_(std::move(base_path)),
      segment_capacity_(segment_capacity) {}

std::vector<std::string> DecoderRowIndexChangedMmap::planned_segment_paths_() const {
    std::vector<std::string> out;
...
int32_t DecoderRowIndexChangedMmap::open_segment_(uint32_t seg_idx) {
    Segment seg;
    const std::string path = make_segment_path(base_path_, seg_idx);
    const size_t size = sizeof(SoAHeader) + static_cast<size_t>(segment_capacity_) * sizeof(uint32_t);
    mmap_create_rw(path.c_str(), size, seg.handle);
    seg.header = reinterpret_cast<SoAHeader*>(seg.handle.addr);
    seg.values = reinterpret_cast<uint32_t*>(reinterpret_cast<uint8_t*>(seg.handle.addr) + sizeof(SoAHeader));
...
int32_t DecoderRowIndexChangedMmap::ensure_segment_(uint64_t global_offset) {
...
int32_t DecoderRowIndexChangedMmap::open_and_init() {
    close_and_finalize();
    if (segment_capacity_ == 0) {
        return file_service::mmap::error_code::kHeaderZeroMetadata;
    }
...
int32_t DecoderRowIndexChangedMmap::write_at(uint64_t global_offset, uint32_t row_index) {
    ...
    segments_[static_cast<size_t>(seg_idx)].values[static_cast<size_t>(local_idx)] = row_index;
    return 0;
}

void DecoderRowIndexChangedMmap::publish_progress(uint64_t written_total, bool done) {
    (void)done;
...
void DecoderRowIndexChangedMmap::close_and_finalize() {
...
std::vector<std::string> DecoderRowIndexChangedMmap::segment_paths() const {
    if (!segments_.empty()) {
...
int32_t DecoderRowIndexChangedMmap::read_total_count(uint64_t& out_total_count) const {
    out_total_count = 0;
...
int32_t DecoderRowIndexChangedMmap::read_value(uint64_t global_offset, uint32_t& out_row_index) const {
    const auto paths = segment_paths();
    return read_u32(paths, global_offset, out_row_index);
}

std::vector<uint32_t> DecoderRowIndexChangedMmap::read_page(int64_t first, int64_t last) const {
