                                    ...
                                    const LogRecord& entry)
{
    ...
    sqlitew::bind_int64(stmt_, 6, changed ? 1 : 0);
...
bool LogIndexDatabase::update_entry(uint32_t row_index,
                                    const LogRecord& entry,
...
std::vector<uint64_t> LogIndexDatabase::query_row_indices(const LogQuery& query,
    ...
    try {
        ...
        while (sqlitew::step(stmt) == SQLITE_ROW) {
            rows.push_back(static_cast<uint64_t>(sqlitew::column_int64(stmt, 0)));
        }
